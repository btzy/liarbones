module.exports=(function(){
    ...
    var get_player_data=function(player,opponent){
        return {MyCoins:player.coins,OpponentCoins:opponent.coins,MyHealth:player.health,OpponentHealth:opponent.health};
    };
    ...
    var clash=function(gameroom){
        ...
        playerdata.Win=(p1.health!=p2.health)?(p1.health>p2.health):(p1.coins>=p2.coins);
        playerdata.Action="END";
        add_to_queue(p1,playerdata);
        playerdata=get_player_data(p2,p1);
        playerdata.NumberOfRounds=games[gameroom].roundnumber;
        playerdata.Win=(p2.health!=p1.health)?(p2.health>p1.health):(p2.coins>=p1.coins);
        playerdata.Action="END";
        add_to_queue(p2,playerdata);
    }
    ...
    var try_bid=function(gameroom,playerid,bid){
        if(games.hasOwnProperty(gameroom)&&games[gameroom].players.hasOwnProperty(playerid)&&Object.keys(games[gameroom].players).length>=2&&games[gameroom].roundstate===1&&bid>=0&&bid<=games[gameroom].players[playerid].coins){
    ...
    var run_generate_dice=function(gameroom){
        for(var playerid in games[gameroom].players){
            if(games[gameroom].players.hasOwnProperty(playerid)){
                games[gameroom].players[playerid].dice=parseInt(Math.random()*6,10)+1;
            }
        }
    ...
    var try_start_game=function(gameroom){
        if(games.hasOwnProperty(gameroom)&&Object.keys(games[gameroom].players).length>=2&&games[gameroom].roundnumber===0){
            start_game(gameroom);
        }
    };
    var ret={};
    ret.init=function(app,path){
        app.post(path+"/listen",function(req,res){
            ...
            if(games.hasOwnProperty(req.body.GameRoom)&&games[req.body.GameRoom].players.hasOwnProperty(req.body.PlayerID)){
        ...
        app.post(path+"/send",function(req,res){
            switch(req.body.Action){
                case "JOIN":
                    if(!req.body.GameRoom||!req.body.PlayerID)return res.send("Missing POST data.");
                    if(!games.hasOwnProperty(req.body.GameRoom))games[req.body.GameRoom]={players:{},roundnumber:0,roundstate:0};
                    if(!games[req.body.GameRoom].players.hasOwnProperty(req.body.PlayerID)&&games[req.body.GameRoom].roundnumber===0){
                        games[req.body.GameRoom].players[req.body.PlayerID]={res:null,coins:20,health:10,bid:-1,dice:-1,listenqueue:[]};
                        try_start_game(req.body.GameRoom);
                    }
                    break;
                case "BID":
                    if(!req.body.GameRoom||!req.body.PlayerID||typeof req.body.Bid==="undefined")return res.send("Missing POST data.");
